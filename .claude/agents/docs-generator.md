...
4. **Follow the architecture**: Documentation must accurately reflect the project's architectural patterns:
   - Services use plain functions + closures (factory pattern), NOT classes
   - Services depend on port interfaces, not adapters
   - Repositories use factory functions returning domain objects, not raw rows
   - Adapters may use classes (need `this` for SDK instances)
   ...
   - Batch external calls use `pMap` with bounded concurrency
   - All config through shared module, never `process.env` directly
...
- [ ] Follows project conventions from CLAUDE.md (12 Hard Rules, 13 Preferred Patterns)
- [ ] Service examples use factory pattern (not classes)
- [ ] Code examples show `readonly` on all interface properties
- [ ] Code examples use `const` (not `let`) unless justified
- [ ] Naming conventions match CLAUDE.md table (kebab-case files, PascalCase types, camelCase functions, SCREAMING_SNAKE constants)
