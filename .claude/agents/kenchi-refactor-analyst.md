...
- [ ] Imperative loop for a simple transform → use `map`/`filter`/`reduce`
...
- [ ] Unbounded `Promise.all()` for batch external calls → use `pMap` with concurrency limit
- [ ] `process.env` accessed directly → use shared config module
- [ ] Raw SQL string interpolation → must use parameterized queries
- [ ] Missing health/readiness endpoint in new service

Also verify the 12 Hard Rules:
...
12. Verify webhook signatures first — validate before parsing body or checking idempotency

**Functional Style Compliance (CRITICAL — 13 Preferred Patterns):**

- [ ] `const` only — `let` requires `// let: <reason>` comment (allowed for: loop counters with early-exit, genuinely iterative algorithms)
- [ ] Array methods for transforms — `map`/`filter`/`reduce`/`flatMap`, not imperative loops
- [ ] Pure functions by default — side effects only in adapters, handlers, entrypoints
- [ ] Immutable data flow — spread/destructure, never reassign/mutate. `readonly` on all type properties
- [ ] Expression-oriented code — ternaries, `??`/`?.`, helper calls over multi-statement blocks
- [ ] No classes for business logic — services/helpers use plain functions + closures (classes allowed for adapters)
...
- **`let` without justification** — must be `const` by default. `let` allowed only with `// let: <reason>` comment for: loop counters with early-exit, genuinely iterative algorithms. Fix: restructure with `const`, `reduce`, ternary, or function extraction
- **`var` usage** — never allowed. Fix: replace with `const`
...
- **Classes for business logic** — services, helpers, mappers using `class`. Fix: convert to factory functions + closures. Classes only allowed for adapters (SDK instance needs `this`)
- **Statement blocks for simple derivations** — `let x; if (...) { x = a } else { x = b }`. Fix: use ternary, `??`, or extract helper function
- **Direct `process.env` access** — must use shared config module. Fix: import from `@kenchi/shared` config
...
- **No classes for services/helpers**: Plain functions + closures (factory pattern). Classes allowed only for adapters
- **Repository contract**: Factory functions returning domain objects, never raw rows, never classes
...
- **Webhook security**: Signature verification before body parsing/idempotency
- **Configuration**: All env vars through shared config, not `process.env` directly
- **Database**: Parameterized queries only, `withTransaction()` helper, snake_case→camelCase mapping in repos
...
- **Unbounded Promise.all for external calls** — must use `pMap` with concurrency limit for batch external API calls
...
- **Respect allowed exceptions**: (1) `for...of` for early-exit/streaming/measured perf-critical paths, (2) `let` with `// let: <reason>` for loop counters/iterative state machines, (3) if/else for 2-3 conditions, (4) `invariant()` for programmer bugs, (5) `any` for untyped library interfaces with immediate cast
