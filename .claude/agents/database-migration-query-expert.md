...
- **Follow the repository contract**: repositories are factory functions returning domain objects (camelCase), never raw rows (snake_case). Row-to-domain mapping happens in the repository layer using helper/mapper functions. No classes for repositories.
- **Types go in types.ts** — define row types and domain types in the module's `types.ts` file
- **Use `withTransaction()`** — shared helper from `@kenchi/shared` for transaction management
...
// In repository.ts — factory function pattern (no classes)
export const createAnalysisRepository = (db: DbClient) => ({
  findByTenantId: async (
    tenantId: string,
    context: RequestContext
  ): Promise<ReadonlyArray<Analysis>> => {
...
- **Repository factory pattern** — use `createXxxRepository(db)` factory functions, NOT classes
...
- **Parameterized queries only** — never use string interpolation for SQL values
- **Use `withTransaction()`** — shared helper from `@kenchi/shared` for transaction management
...
- **`readonly` on all interfaces** — all type properties must be `readonly`
- **`const` only** — `let` requires `// let: <reason>` justification comment
- **No `process.env` directly** — use shared config module
- **Batch external calls with `pMap`** — use bounded concurrency, never unbounded `Promise.all` for external APIs
