...
const DUPLICATION_PATTERNS = [
  ...
  {
    pattern:
      /interface\s+(Config|WebhookEvent|LLMAnalysisResult|CIFailureEvent|SlackMessageEvent|GitHubPREvent)/,
    message: "Duplicate type definition - use types from @kenchi/shared",
    exclude: ["packages/shared"],
  },
  {
    pattern:
      /class\s+(AppError|ValidationError|AuthenticationError|NotFoundError|ExternalServiceError|LLMError)/,
