...
const findBestMatch = (
  ...
  return relatedIncidents.reduce((max, doc) => (doc.similarity > max.similarity ? doc : max), {
    similarity: 0,
    id: "",
  });
};
 ...
 */
const referencesIncident = (analysis: LLMAnalysisResult, incidentId: string): boolean => {
  if (!incidentId) {
    return false;
  }
  ...
  return (
    reasoning.includes(normalizedId) || analysis.relatedIncidents?.includes(incidentId) || false
  );
};
...
export const validateAgainstKnowledgeBase = (
  ...
  if (bestMatch.similarity > SIMILARITY_THRESHOLDS.STRONG && hasReference) {
