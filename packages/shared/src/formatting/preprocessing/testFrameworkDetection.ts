 ...
 *
 * @module formatting/preprocessing/testFrameworkDetection
 */

import { TEST_FRAMEWORK_CONFIDENCE } from "../../constants/index.js";
import type { TestFrameworkInfo, FrameworkPatternEntry } from "./types.js";
...
const calculateConfidence = (
  ...
  const multiMatchBonus =
    matchCount > 1 ? TEST_FRAMEWORK_CONFIDENCE.MULTI_MATCH_BONUS * Math.min(matchCount - 1, 3) : 0;
  const coverageBonus = matchCount === totalPatterns ? 0.05 : 0;
  const finalConfidence = baseConfidence + multiMatchBonus + coverageBonus;

  return Math.min(finalConfidence, TEST_FRAMEWORK_CONFIDENCE.MAX_CONFIDENCE);
};
