 ...
 *
 * @module formatting/common/pathHelpers
 */
...
export const groupByServicePath = <T extends { path: string }>(
  ...
  items.reduce<Map<string, T[]>>((groups, item) => {
    const service = extractServiceFromPath(item.path);
    const existing = groups.get(service) ?? [];
    return new Map(groups).set(service, [...existing, item]);
  }, new Map<string, T[]>());
...
export const formatGroupedItems = <T extends { path: string }>(
  ...
  Array.from(grouped.entries()).flatMap(([service, items]) => {
    const fileCount = items.length === 1 ? "1 file" : `${items.length} files`;
    return [`**${service}** (${fileCount})`, ...items.map(formatItem)];
  });
...
export const buildCanonicalPathMap = (paths: readonly string[]): Map<string, string> => {
  ...
  const baseNameGroups = normalizedPaths.reduce<Map<string, string[]>>((groups, path) => {
    ...
    return new Map(groups).set(baseName, [...existing, path]);
  }, new Map<string, string[]>());

  // Build canonical map from base name groups
  return Array.from(baseNameGroups.values()).reduce<Map<string, string>>(
    (canonicalMap, groupPaths) => {
      ...
      // Map each path to its canonical form
      const pathMappings = uniquePaths.map((path) => {
        const resolvedPath = canonicalPath && !path.includes("/") ? canonicalPath : path;
        return [path, resolvedPath] as const;
      });

      return new Map([...canonicalMap, ...pathMappings]);
    },
    new Map<string, string>()
  );
};
...
export const stripAbsolutePaths = (message: string): string => {
  ...
  return message.replace(ABSOLUTE_PATH_PATTERN, (match) => {
    ...
    const startIndex = parts.findIndex((part) => !ABSOLUTE_PATH_SKIP_DIRS.has(part));
