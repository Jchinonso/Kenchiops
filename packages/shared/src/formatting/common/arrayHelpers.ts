 ...
 * Array Helpers
 *
 * Reusable functions for array manipulation, deduplication,
 * and collection operations.
 *
 * @module formatting/common/arrayHelpers
 */
 ...
 * deduplicateByKey(files, (file) => file.path); // [{ path: 'a.ts' }, { path: 'b.ts' }]
 */
export const deduplicateByKey = <T, K>(
  ...
  return maxItems === undefined ? unique : unique.slice(0, maxItems);
};
 ...
 * groupBy(items, (item) => item.type); // Map { 'a' => [{...}, {...}], 'b' => [{...}] }
 */
