 ...
 * UI Helpers
 *
 * Consistent formatting utilities for confidence scores,
 * colors, text, and list formatting across all services.
 *
 * @module formatting/common/uiHelpers
 */
...
import type { ThresholdEntry, TimeUnit } from "./types.js";
...
/**
 * Confidence range lookup table (descending order for first-match).
 * Uses percentage-based thresholds.
 */
const CONFIDENCE_RANGES: ReadonlyArray<ThresholdEntry<ConfidenceRange>> = [
  { threshold: CONFIDENCE_DISPLAY_THRESHOLDS.HIGH, value: "high" },
  { threshold: CONFIDENCE_DISPLAY_THRESHOLDS.MEDIUM, value: "medium" },
  { threshold: CONFIDENCE_DISPLAY_THRESHOLDS.LOW, value: "low" },
] as const;
...
const getConfidenceRangeForDisplay = (scorePercent: number): ConfidenceRange =>
  findByThreshold(scorePercent, CONFIDENCE_RANGES, "very_low");
