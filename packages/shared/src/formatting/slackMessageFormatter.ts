...
const formatTopIssues = (analysis: LLMAnalysisResult): SlackBlock | null => {
  ...
  const topIssues = annotations.slice(0, 3);
...
export const formatSlackMessage = (
  ...
  const topIssues = formatTopIssues(analysis);
  if (topIssues) {
    blocks.push(topIssues);
  }

  const nextSteps = formatNextSteps(analysis);
  if (nextSteps) {
    blocks.push(nextSteps);
  }

  blocks.push(formatDivider());
  blocks.push(formatActions(context));
