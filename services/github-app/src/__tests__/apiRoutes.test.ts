...
jest.mock("@kenchi/shared", () => ({
  ...
  PRIORITY_ORDER_DEFAULT: 5,
  GITHUB_COMMENT_TEMPLATES: {
    HEADER: "## 🔍 KenchiOps CI Failure Analysis",
    FOOTER: "*Generated by KenchiOps DevOps Assistant*",
  },
  ...
  config: {
    FEEDBACK_BASE_URL: "https://feedback.kenchiops.com",
  },
  generateFeedbackUrl: jest.fn(() => "https://feedback.kenchiops.com/test"),
  extractServiceFromPath: jest.fn((path: string) => {
    if (path.includes("services/api")) return "services/api";
    if (path.includes("services/slack-bot")) return "services/slack-bot";
    if (path.includes("packages/shared")) return "packages/shared";
    return "src";
  }),
  canonicalizeEvidencePaths: jest.fn((testFailures: unknown[], annotations: unknown[]) => ({
    testFailures,
    annotations,
    pathMap: new Map(),
  })),
  clusterFailuresByService: jest.fn(() => new Map()),
  formatWithEvidenceId: jest.fn((text: string) => text),
  selectBestClusterCause: jest.fn(() => "Test cause"),
  formatEvidenceLocation: jest.fn((file: string, line?: number) =>
    line ? `${file}:${line}` : file
  ),
  isLowSignalCause: jest.fn(() => false),
