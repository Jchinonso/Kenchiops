...
const buildConsolidatedSlackPayload = (aggregation: AggregatedFailures): SlackPayload => {
  ...
  const blocks: SlackBlock[] = [
    {
      type: "header",
      text: { type: "plain_text", text: `CI Failure: ${repository}` },
    },
    {
      type: "section",
      fields: [
        { type: "mrkdwn", text: `*Commit:* \`${shortSha}\`` },
        { type: "mrkdwn", text: `*Failed Checks:* ${failureCount}` },
      ],
    },
  ];
  ...
  aggregation.failures.forEach((failure) => {
    ...
    blocks.push({
      type: "section",
      text: { type: "mrkdwn", text: `*${failure.checkName}:*\n${truncatedCause}` },
    });
  });
...
const buildConsolidatedCheckSummary = (aggregation: AggregatedFailures): string => {
  ...
  const lines = [
    ...
    "**Root Causes:**",
    ...causes.map(
      (cause) => `- ${truncateText(cause, GITHUB_COMMENT_DISPLAY.MAX_ANNOTATION_MESSAGE_LENGTH)}`
    ),
  ];
