...
export const ReadyStep = ({ firstName, onComplete, onBack }: ReadyStepProps) => (
  ...
  >
    ...
    <motion.h2
      ...
      className="text-2xl sm:text-3xl font-bold text-zinc-900 dark:text-zinc-100 mb-2"
    >
    ...
    <motion.p
      variants={itemVariants}
      className="text-base text-zinc-500 dark:text-zinc-400 mb-8 max-w-sm mx-auto leading-relaxed"
    >
    ...
    <motion.div variants={itemVariants} className="mt-6">
      <motion.button
        type="button"
        onClick={onBack}
        className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-zinc-400 dark:text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors"
        whileTap={{ scale: 0.97 }}
        transition={microSpring}
      >
